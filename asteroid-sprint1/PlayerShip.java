...
public class PlayerShip extends Actor {
    ...
    private static final double THRUST_POWER = 0.30;  // px/frame^2 (aceleración)
    private static final double DRAG         = 0.985; // fricción leve (1 = sin fricción)
    private static final double MAX_SPEED    = 7.0;   // px/frame (tope)
    private static final double ANGULAR_SPEED= 5.0;   // grados/frame
    ...
    private void shoot() {
        ...
        Greenfoot.playSound("starwars2.wav"); // opcional (si tienes el audio)
    }
    ...
    private void explode() {
        ...
        Greenfoot.playSound("explosion.wav"); // opcional
    ...
    // Cargar tu sprite base desde "images/nave.png"
    imgBase = new GreenfootImage("nave6.PNG");

    // Asegúrate que el tamaño sea razonable (~40x40 px)
    imgBase.scale(40, 40);

    // Crear copia para la versión con llama
    imgThrust = new GreenfootImage(imgBase);

    // Agregar la llama trasera dibujada en código
    int cx = imgThrust.getWidth() / 2;
    int cy = imgThrust.getHeight() / 2;

    imgThrust.setColor(new Color(255, 180, 60)); // naranja
    imgThrust.fillPolygon(
        new int[]{cx - 20, cx - 30, cx - 20},   // x coords
        new int[]{cy - 5,  cy,      cy + 5},    // y coords
        3
    );

    imgThrust.setColor(new Color(255, 100, 0)); // rojo más intenso
    imgThrust.fillPolygon(
        new int[]{cx - 22, cx - 30, cx - 22},
        new int[]{cy - 3,  cy,      cy + 3},
        3
    );
}
